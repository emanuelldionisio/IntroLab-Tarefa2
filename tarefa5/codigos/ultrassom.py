...
def plotar_aceleracao(numero):
    ...
    plt.legend()
    plt.title(f'Posição - Ultrassom {numero}')
    plt.xlabel('Tempo (s)')
    plt.ylabel('Posição (m)')
    plt.grid()
    plt.savefig(f'tarefa5/imgs/Ultrassom{numero}_posicao.png', dpi=300, bbox_inches='tight')
    plt.close()
    #plt.show()
    ...
    plt.legend()
    plt.grid()
    plt.title(f'Posição, velocidade e aceleração - Ultrassom {numero}')
    plt.xlabel('Tempo (s)')
    plt.savefig(f'tarefa5/imgs/Ultrassom{numero}_completo.png', dpi=300, bbox_inches='tight')
    plt.close()
    #plt.show()
