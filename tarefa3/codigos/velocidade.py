...
def plotar_velocidade(numero, posicao):
    dados = []
    ...
    coeficientes, cov = np.polyfit(tempo, velocidade, 1, cov=True)
    velocidadeAjustada = np.polyval(coeficientes, x)
    print(f"Incerteza do ajuste de curva velocidade {numero}: ", np.sqrt(cov[0][0]))

    plt.plot(tempo, velocidade, 'o', label='Velocidade (m/s)', color='blue', markersize=1)
