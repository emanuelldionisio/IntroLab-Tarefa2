...
def plotar_posicao(numero):
    ...
    tempo = [float(linha[0]) for linha in dados if linha[1] != '']
    posicao = [float(linha[1]) for linha in dados if linha[1] != '']
    
    x = np.linspace(min(tempo), max(tempo), 100)
    coeficientes = np.polyfit(tempo, posicao, 2)
    posicaoAjustada = np.polyval(coeficientes, x)
    
    plt.plot(tempo, posicao, 'o', label='Posição (m)', color='blue', linewidth=0.5)
